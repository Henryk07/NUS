...
def plot_state(particles, landmarks, actual_robot_pos, map_limits):
    ...
    plt.quiver(estimated_pose[0], estimated_pose[1], np.cos(estimated_pose[2]), np.sin(
        estimated_pose[2]), angles='xy', scale_units='xy', color='g')
    plt.quiver(actual_robot_pos[0], actual_robot_pos[1], np.cos(
        actual_robot_pos[2]), np.sin(actual_robot_pos[2]), angles='xy', scale_units='xy')
    plt.axis(map_limits)
...
def main():
    ...
    landmarks = read_world("P_Filter/ESP-particleFilter-assignment/world.dat")
    ...
    for iteration in range(100):
        ...
        actual_robot_pos = update_robot_pos(
            actual_robot_pos, odometry, map_limits)
        ...
        pred_error = np.sqrt(
            (predicted_pos[0] - actual_robot_pos[0])**2 + (predicted_pos[1] - actual_robot_pos[1])**2)
